...
def init(self):
    ...
    def oncreate(self, old_event = None):
        #print("file:幻影狼 ,card:%s ,buff:%s ,function:create" % (buff.card.name, buff.name))
        self.effectbuff = []
        self.effectcard = []
        for card in self.system.cards:
            if (card.player == self.card.player) and (card.place == PLACE_FIELD) and (SUBTYPE_BEAST in card.subtype):
                new_buff = Buff(self.system, "b0000000004_002", self.card, card)
    ...
    def onremove(self, old_event = None):
        #print("file:幻影狼 ,card:%s ,buff:%s ,function:onremove" % (buff.card.name, buff.name))
